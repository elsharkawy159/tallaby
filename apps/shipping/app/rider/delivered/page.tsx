...
import { getRiderDeliveredToday } from "../rider.server";

export const dynamic = "force-dynamic";

async function DeliveredList() {
  const t = await getTranslations("rider");
  const result = await getRiderDeliveredToday();
  ...
  if (deliveries.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl border bg-white p-10 text-center dark:bg-gray-950">
        ...
        <p className="text-sm text-muted-foreground">{t("noDeliveredToday")}</p>
      </div>
    );
  }
...
export default async function RiderDeliveredPage() {
  const t = await getTranslations("rider");
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold tracking-tight">{t("deliveredToday")}</h1>
      </div>
