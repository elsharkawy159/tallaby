...
export const Sidebar = ({ isOpen, onToggle }: SidebarProps) => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className={cn(
        "hidden lg:flex max-w-72 lg:flex-col lg:fixed lg:inset-y-0 bg-primary dark:bg-primary z-50 transition-all duration-300",
    ...
    <div className="flex flex-col h-full">
      ...
      {!isCollapsed && (
        <div className="flex flex-col items-center py-8 px-6 border-b border-primary-foreground/20">
          ...
          <h2 className="text-white font-semibold text-lg mb-1">{t('user.johnDoe')}</h2>
          <p className="text-primary-foreground/80 text-sm text-center">{t('sidebar.premiumElectronicsStore')}</p>
                ...
                >
                  <item.icon 
                    className={cn(
                      ...
                      isActive ? "text-primary" : "text-primary-foreground/70 group-hover:text-white",
                      !isCollapsed && "mr-3"
                    )} 
                  />
